...
public class DosageFilter implements Filter {
	...
	@Override
	public List<Boolean> batchPassesFilter(List<IdentifierTarget> targets, HouselistEntry entry) {
		Map<Long, IdentifierTarget> targetIds = new HashMap<>(targets.size());
		for (IdentifierTarget t : targets) {
			if (t.type == IdentifierTarget.Type.PRODUCT)
				targetIds.put(t.mmiId, t);
		}
		...
		while (result.hasNext()) {
			...
			IdentifierTarget target = targetIds.get(productId);
	...
	public boolean passesFilter(IdentifierTarget target, HouselistEntry entry) {
		if (target.type != IdentifierTarget.Type.PRODUCT) return false;

		Result result = queryEffectiveDosagesForProductIds(List.of(target.mmiId));
	...
	private static class ExportDosage {
		...
		/**
		 * Returns whether the given dosage nominator matches this instance.
		 * If the given dosage does not provide a unit, then no unit comparison is performed, i.e. the unit is always
		 * considered to match. But if a unit is provided, it must match.
		 */
		private boolean matchesAbsolute(Dosage dosage) {
			Amount amount = dosage.amountNominator;
			if (!matchesAbsolute(amountFrom, amountTo, amount.number, BigDecimal.ZERO)) return false;
		...
		private boolean matchesAbsolute(BigDecimal from, BigDecimal to, BigDecimal target, BigDecimal delta) {
			if (target == null) return false;
			if (amountTo == null) {
				if (amountFrom.subtract(target).abs().compareTo(delta) > 0) return false;
			} else {
				if (amountFrom.compareTo(target) > 0 || amountTo.compareTo(target) < 0) return false;
			}
		...
		/**
		 * Returns whether the given dosage fraction matches this instance and the given drug.
		 * For example, if the drug is 3ml and this unit is 300mg, this would translate to 100mg/ml.
		 * If that is the value provided by the dosage (or sth else like 300mg/3ml), it is considered a match.
		 * The units must match perfectly in this case, unlike in the absolute matching.
		 */
		private boolean matchesRelative(Dosage dosage, ExportAmount drugAmount) {
			...
			BigDecimal normalizedAmountLocalFrom = amountFrom.setScale(4, RoundingMode.UNNECESSARY).divide(drugAmount.amount, RoundingMode.HALF_UP);
			BigDecimal normalizedAmountLocalTo = null;
			if (amountTo != null)
				normalizedAmountLocalTo = amountTo.setScale(4, RoundingMode.UNNECESSARY).divide(drugAmount.amount, RoundingMode.HALF_UP);
			BigDecimal normalizedAmountOther = dosage.amountNominator.number.setScale(4, RoundingMode.HALF_UP).divide(dosage.amountDenominator.number, RoundingMode.HALF_UP);

			return matchesAbsolute(normalizedAmountLocalFrom, normalizedAmountLocalTo, normalizedAmountOther, EPSILON);
		}

	}

	// TODO We need to normalize units
