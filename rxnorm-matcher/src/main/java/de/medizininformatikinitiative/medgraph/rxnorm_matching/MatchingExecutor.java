...
public class MatchingExecutor {
	...
	public static List<RxNormProductMatcher2.MatchResult> matchAll(Collection<Drug> drugs,
		...
		try (ForkJoinPool threadPool = new ForkJoinPool(threads)) {
			...
			BlockingQueue<RxNormProductMatcher2.MatchResult> results = new LinkedBlockingQueue<>();

			if (!silent) System.out.println("Retrieved " + numDrugs + " drugs. Starting matching.");
			threadPool.submit(() ->
					drugs.stream()
					     .parallel()
					     .map(matcher2::matchSCD)
					     .forEach(results::add)
			);

			List<RxNormProductMatcher2.MatchResult> allResults = new ArrayList<>(numDrugs);
			int successful = 0;
			for (int i = 1; i <= numDrugs; i++) {
				RxNormProductMatcher2.MatchResult result = results.poll(5, TimeUnit.SECONDS);
...
//			results.forEach(System.out::println);
			List<RxNormProductMatcher2.MatchResult> successes = allResults.stream()
