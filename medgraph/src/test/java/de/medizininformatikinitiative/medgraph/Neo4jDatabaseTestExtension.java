...
public class Neo4jDatabaseTestExtension implements BeforeAllCallback, ExtensionContext.Store.CloseableResource {
	...
	public static String getDatabaseFixture() {
		...
		return """
				...
				(n82:EDQM:Code {code: "BDF-0083", name: "Solution", status: "Current", type: "Basic dose form"})<-[:HAS_CHARACTERISTIC]-(n46:EDQM:Code:DoseForm {code: "PDF-11201000", name: "Solution for injection", status: "Current", type: "Pharmaceutical dose form"})-[:HAS_CHARACTERISTIC]->(n80:EDQM:Code {code: "ISI-0033", name: "Parenteral", status: "Current", type: "Intended site"})<-[:HAS_CHARACTERISTIC]-(n26:EDQM:Code:DoseForm {code: "PDF-50060000", name: "Solution for injection/infusion", status: "Current", type: "Pharmaceutical dose form"})<-[:CORRESPONDS_TO]-(n30:MmiDoseForm {mmiCode: 518, mmiName: "Injektions-/Infusionslsg.", mmiDesc: "parenteral"})<-[:HAS_DOSE_FORM]-(n29:Drug {amount: 3, mmiId: 2})-[:CONTAINS]->(n23)-[:IS_SUBSTANCE]->(:Substance {name: "Midazolam hydrochlorid", mmiId: 3})<-[:IS_SUBSTANCE]-(n34:MmiIngredient:Ingredient {isActive: "true", massFrom: "5,5", massTo: "5,7", mmiId: 3})-[:HAS_UNIT]->(n6)<-[:HAS_UNIT]-(n37),
				...
				(n70)-[:HAS_DOSE_FORM]->(:MmiDoseForm {mmiCode: 432, mmiName: "Pulver zur Herst. e. Inj.-Lsg.", mmiDesc: "parenteral"})-[:CORRESPONDS_TO]->(n75)-[:HAS_CHARACTERISTIC]->(n80),
				...
				(:EDQM:Code {code: "ISI-0031", name: "Oral", status: "Current", type: "Intended site"})<-[:HAS_CHARACTERISTIC]-(n5)-[:HAS_CHARACTERISTIC]->(:EDQM:Code {code: "BDF-0053", name: "Granules", status: "Current", type: "Basic dose form"})
				"""
				.stripIndent()
