...
public class Neo4jDatabaseTestExtension implements BeforeAllCallback, ExtensionContext.Store.CloseableResource {
	...
	private static String getDatabaseFixture() {
		...
		return """
				...
				(n48)<-[:IS_SUBSTANCE]-(n51:MmiIngredient:Ingredient {isActive: "false", massFrom: 3, mmiId: 5})-[:HAS_UNIT]->(n32)<-[:HAS_UNIT]-(n44)-[:HAS_DOSE_FORM]->(:DoseForm {mmiCode: 220, mmiName: "Injektionslsg.", mmiDesc: "parenteral"})-[:CORRESPONDS_TO]->(:EDQM:Code {code: "PDF-11201000", name: "Solution for injection", status: "Current", intendedSite: "Parenteral"}),
				...
				(:Substance {name: "Prednisolon 21-hydrogensuccinat, Natriumsalz", mmiId: 7})<-[:IS_SUBSTANCE]-(n76)<-[:CONTAINS]-(n70:Drug {amount: 1, mmiId: 6})<-[:CONTAINS]-(n67:Product {name: "Prednisolut® 10 mg L, Pulver und Lösungsmittel zur Herstellung einer Injektionslösung", mmiId: 6})-[:CONTAINS]->(n68)-[:HAS_DOSE_FORM]->(:DoseForm {mmiCode: 432, mmiName: "Lösungsmittel"}),
				(n70)-[:HAS_DOSE_FORM]->(:DoseForm {mmiCode: 432, mmiName: "Pulver zur Herst. e. Inj.-Lsg.", mmiDesc: "parenteral"})-[:CORRESPONDS_TO]->(:EDQM:Code {code: "PDF-11205000", name: "Powder for solution for injection", status: "Current", intendedSite: "Parenteral"}),
