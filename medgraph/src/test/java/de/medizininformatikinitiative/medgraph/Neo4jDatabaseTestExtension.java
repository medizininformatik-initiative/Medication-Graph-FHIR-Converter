...
public class Neo4jDatabaseTestExtension implements BeforeAllCallback, ExtensionContext.Store.CloseableResource {
	...
	private static String getDatabaseFixture() {
		...
		return """
                ...
                (n30)<-[:HAS_DOSE_FORM]-(n36:Drug {amount: 5, mmiId: 3})<-[:CONTAINS]-(:Product {name: "Dormicum® V 5 mg/5 ml Injektionslösung", mmiId: 3})<-[:REFERENCES]-(:Name {name: "Dormicum"})-[:REFERENCES]->(n22:Product {name: "Dormicum® 15 mg/3 ml Injektionslösung", mmiId: 2})-[:CONTAINS]->(n29)-[:HAS_UNIT]->(n32:Unit:UCUM {print: "ml", ucumCs: "ml", ucumCi: "ML", mmiCode: "ML", mmiName: "ml"})<-[:HAS_UNIT]-(n36)-[:CONTAINS]->(:MmiIngredient:Ingredient {isActive: "false", massFrom: 5, mmiId: 6}),
                (n56:Package {amount: 5, `amountFactor1`: 1, `amountFactor2`: 1, amountText: "5 Ampullen", mmiId: 3, name: "Dormicum 5mg Ampullen", onMarketDate: "2020-01-01"})-[:BELONGS_TO]->(n36)-[:CONTAINS]->(n34)-[:CORRESPONDS_TO]->(n39:Ingredient {massFrom: 5})-[:HAS_UNIT]->(n6),
                (n39)-[:IS_SUBSTANCE]->(n38),
                (:Name {name: "Adrenalin"})-[:REFERENCES]->(:Substance {name: "Epinephrin", mmiId: 4})<-[:IS_SUBSTANCE]-(n43:MmiIngredient:Ingredient {isActive: "true", massFrom: 300, mmiId: 4})<-[:CONTAINS]-(n44:Drug {amount: "0,3", mmiId: 4})<-[:CONTAINS]-(n45:Product {name: "Anapen 300 µg kohlpharma Injektionslösung", mmiId: 4})<-[:BELNOGS_TO]-(n58:Package {amount: 1, `amountFactor1`: 1, `amountFactor2`: 1, amountText: "1 st", mmiId: 4, name: "Anapen 300ug", onMarketDate: "2022-07-15"}),
                (n43)-[:HAS_UNIT]->(:Unit:UCUM {print: "μg", ucumCs: "ug", ucumCi: "UG", mmiCode: "MCG", mmiName: "μg"}),
                (n51:MmiIngredient:Ingredient {isActive: "false", massFrom: 3, mmiId: 5})-[:HAS_UNIT]->(n32)<-[:HAS_UNIT]-(n44)-[:HAS_DOSE_FORM]->(:DoseForm {mmiCode: 220, mmiName: "Injektionslsg.", mmiDesc: "parenteral"})-[:CORRESPONDS_TO]->(:EDQM:Code {code: "PDF-11201000", name: "Solution for injection", status: "Current", intendedSite: "Parenteral"}),
                (:Name {name: "Wasser"})-[:REFERENCES]->(:Substance {name: "Wasser für Injektionszwecke", mmiId: 5}),
                (n29)-[:CONTAINS]->(n51),
                ...
                (:PZN:Code {code: 10000003})-[:REFERENCES]->(n56),
