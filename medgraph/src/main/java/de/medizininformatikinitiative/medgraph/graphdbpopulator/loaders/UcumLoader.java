...
/**
 * Applies the UCUM translations to all units where they are known.
 *
 * @author Markus Budeus
 */
public class UcumLoader extends Loader {
	...
	static {
		...
		// Same as in MMI Pharmindex
		// Well, not exactly. The MMI Pharmindex uses the uppercase "l" for a case-sensitive "liter", but both
		// are valid. Lowercase l is more common in europe, so I went with that.
		...
		ucumDefinitions.put("MCKAT", new UcumDefinition("ukat", "UKAT", "μkat"));
		...
		ucumDefinitions.put("USPE", new UcumDefinition("[USP'U]", "[USP'U]", "U.S.P."));
		...
		ucumDefinitions.put("BIO VEKTORGENOM",
				new UcumDefinition("{Bio. Vectorgenome}", "{BIO. VECTORGENOME}", "Bio. Vectorgenome"));
		// ACHTUNG ACHTUNG ACHTUNG
		// Im MMI-Code "ΜG (MW)" ist das erste Zeichen kein "M"! Es hat den Binärcode 0x039C, der in UTF-8 lediglich wie ein "M" aussieht!!
		ucumDefinitions.put("ΜG (MW)", new UcumDefinition("ug{(MW)}", "UG{(MW)}", "μg (MW)"));
		// --------------------------------------------------------------------------------------------------------------------------
	...
	public UcumLoader(Session session) {
		super(session);
	}
	...
	protected void executeLoad() {
		...
		executeQuery("MATCH (v1:" + UNIT_LABEL + " {mmiCode: 'VOLPERC'}) " +
