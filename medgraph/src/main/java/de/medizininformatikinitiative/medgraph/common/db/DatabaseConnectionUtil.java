...
public class DatabaseConnectionUtil {
	...
	throws DatabaseConnectionException {
		...
		try {
			...
			throw new DatabaseConnectionException(ConnectionResult.INVALID_CONNECTION_STRING,
		...
		try (Session session = connection.createSession()) {
			...
			if (e instanceof AuthenticationException) {
				throw new DatabaseConnectionException(ConnectionResult.AUTHENTICATION_FAILED,
						"Authentication failed! (" + e.getMessage() + ")");
			} else if (e instanceof Neo4jException) {
				throw new DatabaseConnectionException(ConnectionResult.SERVICE_UNAVAILABLE,
						"Neo Neo4j service is reachable at " + connectionConfiguration.getUri() + ". (" + e.getMessage() + ")");
			} else {
				throw new DatabaseConnectionException(ConnectionResult.INTERNAL_ERROR,
