...
public class ApacheLuceneInitialMatchFinder_V4_MedicalAnalyzer implements InitialMatchFinder<Product>, Closeable {
    private final Session session;
    private final Directory index;


    public ApacheLuceneInitialMatchFinder_V4_MedicalAnalyzer(Session session) {
    ...
    /**
     * Indexes all products in the database.    */
    private void indexProducts() {
        try (IndexWriter writer = new IndexWriter(index, new IndexWriterConfig(new MedicalCustomAnalyzer()))) {
            ...
            result.stream().forEach(record -> {
                ...
                doc.add(new StoredField("id", id)); // Speichern für Abrufbarkeit
    ...
    public Stream<OriginalMatch<Product>> findInitialMatches(SearchQuery query) {
        ...
        try (DirectoryReader reader = DirectoryReader.open(index)) { // Reader wird hier jedes Mal geöffnet
            IndexSearcher searcher = new IndexSearcher(reader); // Neuer IndexSearcher bei jedem Aufruf
            TopDocs topDocs = searcher.search(new QueryParser("name", new StandardAnalyzer()).parse(searchTerm), 8);
