...
public class ApacheLuceneInitialMatchFinder_V1_unoptimiert implements InitialMatchFinder<Product>, Closeable {
	private final Session session;
	private final Directory index;

	public ApacheLuceneInitialMatchFinder_V1_unoptimiert(Session session) {
	...
	public Stream<OriginalMatch<Product>> findInitialMatches(SearchQuery query) {
		...
		try (DirectoryReader reader = DirectoryReader.open(index)) { // new DirectoryReader for each call
			IndexSearcher searcher = new IndexSearcher(reader); // new IndexSearcher for each call
	...
	public void close() throws IOException {
		index.close(); // Schließt den ByteBuffersDirectory und gibt Ressourcen frei
	}
