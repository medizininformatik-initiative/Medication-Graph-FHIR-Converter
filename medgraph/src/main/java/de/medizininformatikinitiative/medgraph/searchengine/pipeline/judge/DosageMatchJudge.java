...
/**
 * Compares dosage information in a {@link SearchQuery} against dosage information found in the knowledge graph
 * for current matches to assign a score based on how well they overlap.
 *
 * @author Markus Budeus
 */
public class DosageMatchJudge extends ScoreJudge {
	...
	/**
	 * The minimum score which is applied if there is any kind of match. Without any match, the score is zero.
	 */
	public static final double MIN_SCORE_ON_MATCH = DOSAGELESS_SCORE;

	private final Session session;

	public DosageMatchJudge(Session session, Double passingScore) {
		super(passingScore);
	...
	public String getDescription() {
		return "Assigns scores to product matches based on how well their active ingredient dosages match the search.";
	}

	@Override
	protected List<Double> batchJudgeInternal(List<Matchable> targets, SearchQuery entry) {
		if (entry.getActiveIngredientDosages().isEmpty())
		...
		while (result.hasNext()) {
			...
			Matchable target = targetIds.get(productId);
			if (target != null) {
	...
	@Override
	protected double judgeInternal(Matchable target, SearchQuery entry) {
		if (!(target instanceof Product)) return NO_PRODUCT_SCORE;
		if (entry.getActiveIngredientDosages().isEmpty()) return DOSAGELESS_SCORE;

		Result result = queryEffectiveDosagesForProductIds(List.of(((Product) target).getId()));
		if (!result.hasNext()) return 0.0;
		return judge(result.next(), entry);
	}

	private double judge(Record record, SearchQuery entry) {
		List<Dosage> unmatchedDosages = new ArrayList<>(entry.getActiveIngredientDosages());
