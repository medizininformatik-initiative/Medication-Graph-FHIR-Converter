...
public class ApacheLuceneInitialMatchFinder_V2_Abfrageoptimierung implements InitialMatchFinder<Product>, Closeable {
    ...
    public ApacheLuceneInitialMatchFinder_V2_Abfrageoptimierung(Session session) {
        this.session = session;
        this.index = new ByteBuffersDirectory();
        indexProducts();

        try {
            this.reader = DirectoryReader.open(index); // DirectoryReader wird nur einmal geöffnet
            this.searcher = new IndexSearcher(reader); // IndexSearcher wird nur einmal erstellt
        } catch (IOException e) {
