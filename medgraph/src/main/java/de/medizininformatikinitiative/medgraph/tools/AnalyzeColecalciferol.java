...
/**
 * Detaillierte Analyse: Colecalciferol
 */
public class AnalyzeColecalciferol {
    ...
    public static void main(String[] args) {
        ...
        try (Driver driver = GraphDatabase.driver(uri, AuthTokens.basic(user, password));
            ...
            System.out.println("=== Colecalciferol Coverage Problem-Analyse ===\n");
            
            // Step 1: Finde Substanz
            System.out.println("Step 1: Finde Colecalciferol in Neo4j...");
            Long substanceMmiId = findSubstance(session, "Colecalciferol");
            if (substanceMmiId == null) {
                System.out.println("  ✗ Colecalciferol nicht gefunden!");
                return;
            }
            ...
            if (rxcuis.isEmpty()) {
                ...
                // Prüfe ob RxCUIs SCDs haben
                for (String rxcui : rxcuis) {
                    boolean hasScds = provider.hasScdsForIngredient(rxcui);
                    System.out.println("    RxCUI " + rxcui + ": " + (hasScds ? "✓ hat SCDs" : "✗ keine SCDs"));
                }
            }
            ...
            // Step 5: Analysiere Kombinationspräparate
            System.out.println("\nStep 5: Analysiere Kombinationspräparate...");
            analyzeCombinationDrugs(session, substanceMmiId);
            
            // Step 6: Analysiere Sample Drugs
            System.out.println("\nStep 6: Analysiere Sample Drugs (warum Matching scheitert)...");
            List<DrugSample> drugs = findDrugs(session, substanceMmiId, 30);
            ...
            // Step 7: Teste Matching für Sample Drugs
            System.out.println("\nStep 7: Teste Matching für Sample Drugs...");
            ...
            int ingredientMismatch = 0;
            int matched = 0;
            
            for (int i = 0; i < Math.min(30, drugs.size()); i++) {
                ...
                if (result == null) {
                    ...
                    if (drug.drug.ingredients().isEmpty()) {
                        ...
                        if (!hasRxCUIs) {
                            ...
                            noCandidates++;
                            System.out.println("    ✗ REJECTED: Keine SCD-Kandidaten gefunden oder Strength/Ingredient-Mismatch");
                        }
                    }
                } else {
                    matched++;
            ...
            System.out.println("\n=== Zusammenfassung ===");
            System.out.println("Total Drugs: " + totalDrugs);
            System.out.println("Sample analysiert: " + Math.min(30, drugs.size()));
            ...
            System.out.println("  - Ingredient-Mismatch: " + ingredientMismatch);
    ...
    private static void analyzeCombinationDrugs(Session session, Long substanceMmiId) {
        String query = """
            MATCH (s:Substance {mmiId: $mmiId})
            MATCH (s)<-[:IS_SUBSTANCE]-(i:MmiIngredient {isActive: true})
            MATCH (i)<-[:CONTAINS]-(d:Drug)
            MATCH (d)-[:CONTAINS]->(otherIng:MmiIngredient {isActive: true})
            WHERE otherIng <> i
            WITH d, collect(DISTINCT otherIng) AS otherIngs
            RETURN count(DISTINCT d) AS combinationDrugs,
                   collect(DISTINCT size(otherIngs)) AS ingredientCounts
            """;
        
        Result result = session.run(query, Map.of("mmiId", substanceMmiId));
        if (result.hasNext()) {
            var record = result.next();
            int combDrugs = record.get("combinationDrugs").asInt();
            System.out.println("  Kombinationspräparate: " + combDrugs);
            
            // Prüfe welche anderen Ingredients verwendet werden
            String query2 = """
                MATCH (s:Substance {mmiId: $mmiId})
                MATCH (s)<-[:IS_SUBSTANCE]-(i:MmiIngredient {isActive: true})
                MATCH (i)<-[:CONTAINS]-(d:Drug)
                ...
                WHERE otherIng <> i
                RETURN otherSub.name AS otherSubstance, count(DISTINCT d) AS drugCount
                ORDER BY drugCount DESC
                LIMIT 10
                """;
            
            Result result2 = session.run(query2, Map.of("mmiId", substanceMmiId));
            System.out.println("  Häufigste Kombinationspartner:");
            while (result2.hasNext()) {
                var record2 = result2.next();
                String otherSub = record2.get("otherSubstance").asString();
                int count = record2.get("drugCount").asInt();
                System.out.println("    - " + otherSub + " (" + count + " Drugs)");
            }
        }
    }
